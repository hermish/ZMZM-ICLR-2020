...
% Riemannian subgradient descent (random orthogonal dictionary)
% Update: 
...
max_iter = 3500;
eps = 10^-3;   % neighborhood size (stopping criterion)
...
for k = 1:length(p)
	...
	for j = 1:10  % 10 random instances
		...
		Q = randU(n);     % a uniformly random orthogonal matrix
		...
		for l = 1:round(5*n*log(n))
			...
			for i = 1:max_iter
				...
				q_rot = Q' *q;    % should be 1-sparse if one dictionary element is found
